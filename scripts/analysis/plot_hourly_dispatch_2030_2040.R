...
## ---- residual demand duration curve + total energy -------------------------
# Residual demand = demand - wind - solar (the load left after VRE). "After
# storage" additionally subtracts net storage output (discharge - charge, incl.
# hydrogen), so the peak is shaved and the surplus tail lifted toward zero.
# Each series is sorted high-to-low to give a duration curve.

rldc <- rbindlist(lapply(parts, function(p) {
  res_pre  <- p$demand - p$gens$wind - p$gens$solar
  sto_net  <- rowSums(as.matrix(p$stor[, setdiff(names(p$stor), "time"), with = FALSE]))
  res_post <- res_pre - sto_net
  n <- length(res_pre)
  data.table(pct = 100 * (seq_len(n) - 0.5) / n,
             `residual demand` = sort(res_pre,  decreasing = TRUE),
             `after storage`   = sort(res_post, decreasing = TRUE),
             year = p$year)
}))
rldc_long <- melt(rldc, id.vars = c("pct", "year"),
                  variable.name = "series", value.name = "GW")

fig_rldc <- ggplot(rldc_long, aes(pct, GW, colour = series)) +
  geom_hline(yintercept = 0, linewidth = 0.3, colour = "grey60") +
  geom_line(linewidth = 0.7) +
  facet_grid(. ~ year) +
  scale_colour_manual(values = c("residual demand" = d3[4], "after storage" = d3[1]),
                      name = NULL) +
  labs(x = "% of hours (sorted, highest to lowest)",
       y = "Residual demand (GW)") +
  base_theme + theme(legend.position = "bottom")

# total annual energy by technology (TWh): all generation + hydrogen + storage
# discharge, plus demand as a reference bar; grouped by year.
energy <- rbindlist(lapply(parts, function(p) {
  sd <- pmax(rowSums(as.matrix(p$stor[, setdiff(names(p$stor), "time"), with = FALSE])), 0)
  e <- melt(p$gens, id.vars = "time", variable.name = "technology",
            value.name = "GW")[, .(TWh = sum(GW) / 1000), by = technology]
  rbind(e,
        data.table(technology = "H2 turbine",        TWh = sum(pmax(p$h2_turbine, 0)) / 1000),
        data.table(technology = "storage discharge", TWh = sum(sd) / 1000),
        data.table(technology = "demand",            TWh = sum(p$demand) / 1000))[
    , year := p$year][]
}))
energy[, technology := factor(technology, levels = c("demand", gen_techs))]

fig_energy <- ggplot(energy[!is.na(technology)], aes(technology, TWh, fill = year)) +
  geom_col(position = position_dodge(width = 0.75), width = 0.7) +
  scale_fill_manual(values = c("2030" = d3[11], "2040" = d3[1]), name = NULL) +
  labs(x = NULL, y = "Annual energy (TWh)") +
  base_theme + theme(axis.text.x = element_text(angle = 30, hjust = 1),
                     legend.position = "bottom")

resid_fig <- (fig_rldc / fig_energy) +
  plot_layout(heights = c(1, 0.85)) +
  plot_annotation(tag_levels = "a", tag_prefix = "(", tag_suffix = ")")
ggsave(file.path(OUT_DIR, "residual_demand_and_energy.pdf"), resid_fig,
       width = 8.27, height = 8, units = "in", device = cairo_pdf)
ggsave(file.path(OUT_DIR, "residual_demand_and_energy.png"), resid_fig,
       width = 8.27, height = 8, units = "in", dpi = 150)
message("wrote residual_demand_and_energy.pdf / .png")

## ---- residual demand duration curve: with/without storage, battery vs LDES --
# residual = demand - wind - solar. Storage lowers it: battery first, then the
# other long-duration stores (pumped hydro, LAES, hydrogen). The coloured bands
# between the "no storage" and "with storage" curves are each one's contribution;
# positive-area = annual residual demand, negative-area = annual surplus (TWh/yr).
res_pos <- function(x) sum(pmax(x, 0)) / 1000    # TWh/yr residual demand
res_neg <- function(x) -sum(pmin(x, 0)) / 1000   # TWh/yr surplus (magnitude)
rlist <- lapply(parts, function(p) {
  res  <- p$demand - p$gens$wind - p$gens$solar
  bess <- p$stor$BESS
  ldes <- rowSums(as.matrix(p$stor[, intersect(c("pumped hydro", "LAES", "Hydrogen"),
                                               names(p$stor)), with = FALSE]))
  res_b <- res - bess            # after battery
  res_a <- res - bess - ldes     # after battery + other LDES
  n <- length(res)
  dt <- data.table(pct  = 100 * (seq_len(n) - 0.5) / n,
                   pre  = sort(res,   decreasing = TRUE),
                   mid  = sort(res_b, decreasing = TRUE),
                   post = sort(res_a, decreasing = TRUE),
                   year = p$year)
  st <- data.table(year = p$year, rd = res_pos(res), su = res_neg(res),
                   b_rd = res_pos(res)   - res_pos(res_b), b_su = res_neg(res)   - res_neg(res_b),
                   l_rd = res_pos(res_b) - res_pos(res_a), l_su = res_neg(res_b) - res_neg(res_a))
  list(dt = dt, st = st)
})
rl_dt <- rbindlist(lapply(rlist, `[[`, "dt"))
rl_st <- rbindlist(lapply(rlist, `[[`, "st")); print(rl_st)
rl_lab <- rl_st[, .(year, label = sprintf(paste0(
  "Residual demand: %.0f TWh/yr\nSurplus: %.0f TWh/yr\n\nLowered by:\n",
  "  battery  –%.0f demand, –%.0f surplus\n  other LDES  –%.0f demand, –%.0f surplus"),
  rd, su, b_rd, b_su, l_rd, l_su))]

pal_band <- c("battery" = "#6baed6", "other LDES" = "#fd8d3c")
fig_res <- ggplot(rl_dt, aes(pct)) +
  geom_ribbon(aes(ymin = pmin(pre, mid),  ymax = pmax(pre, mid),  fill = "battery")) +
  geom_ribbon(aes(ymin = pmin(mid, post), ymax = pmax(mid, post), fill = "other LDES")) +
  geom_hline(yintercept = 0, linewidth = 0.4, colour = "grey45") +
  geom_line(aes(y = pre,  linetype = "no storage"),   linewidth = 0.9) +
  geom_line(aes(y = post, linetype = "with storage"), linewidth = 0.9) +
  geom_text(data = rl_lab, aes(x = 33, y = 62, label = label), hjust = 0, vjust = 1,
            size = 3.5, lineheight = 0.98, inherit.aes = FALSE) +
  facet_grid(. ~ year) +
  scale_fill_manual(name = "storage lowers residual demand:", values = pal_band) +
  scale_linetype_manual(name = "residual demand duration curve:",
                        values = c("no storage" = "solid", "with storage" = "22")) +
  labs(x = "Share of hours (%), sorted highest to lowest",
       y = "Residual demand (GW)") +
  theme_bw(base_size = 13) +
  theme(legend.position = "bottom", legend.box = "vertical", legend.margin = margin(2, 2, 2, 2),
        strip.text = element_text(face = "bold", size = 13),
        axis.title = element_text(size = 13), panel.grid.minor = element_blank())
ggsave(file.path(OUT_DIR, "residual_demand_storage.pdf"), fig_res,
       width = 10, height = 6.2, units = "in", device = cairo_pdf)
ggsave(file.path(OUT_DIR, "residual_demand_storage.png"), fig_res,
       width = 10, height = 6.2, units = "in", dpi = 160)
message("wrote residual_demand_storage.pdf / .png")
